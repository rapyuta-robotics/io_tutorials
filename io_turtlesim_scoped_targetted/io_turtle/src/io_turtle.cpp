...
void IOTurtle::register_turtle(ros::NodeHandle& nh, bool sim, int queue_size) {
    _sim = sim;
    ...
    if (_sim) {
        ...
        _sim_velocity_pub = nh.advertise<geometry_msgs::Twist>("sim/cmd_vel", 1);
    }

    _velocity_sub = nh.subscribe("cmd_vel", queue_size, &IOTurtle::velocity_callback, this);
    _pose_pub = nh.advertise<geometry_msgs::Pose2D>("pose", 1);
}

void IOTurtle::set_goal(float x, float y) {
    ROS_INFO("Request for Turtle to move to (%f, %f) is received.", x, y);
    _goal_x = x;
    _goal_y = y;
}
...
void IOTurtle::move_towards_goal() {
    ...
    if (_turtle._current_vel.linear.x >= 0.0) {
...
bool IOTurtle::is_moving() const {
    return std::fabs(_turtle._current_vel.linear.x) > 1e-4f || std::fabs(_turtle._current_vel.angular.z) > 1e-4f;
}
...
bool IOTurtle::check_collision() const {
    ...
    return ((_turtle._current_vel.linear.x> 0.0 && _turtle._sensors.front < _collision_range) ||
            (_turtle._current_vel.linear.x < 0.0 && _turtle._sensors.rear < _collision_range));
}
...
void IOTurtle::velocity_callback(const geometry_msgs::Twist& vel) {
    if ((std::fabs(vel.linear.x) < 1e-4f && std::fabs(vel.angular.z) < 1e-4f)) {
...
void IOTurtle::sim_pose_callback(const geometry_msgs::Pose2D& pose) {
    _turtle._pose = pose;
    _pose_pub.publish(_turtle._pose);
}

void IOTurtle::sim_sensors_callback(const io_turtle_scoped_targetted_msgs::DistanceSensor& sensors) {
