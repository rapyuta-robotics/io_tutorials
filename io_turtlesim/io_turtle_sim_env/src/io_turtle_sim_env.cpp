...
bool SimEnv::teleport_turtle(TeleportTurtle::Request& req, TeleportTurtle::Response& response) {
...
// SimTurtle::SimTurtle(ros::NodeHandle& nh, ros::NodeHandle& priv_nh, int id, TurtleGeometryMsg turtle, 
//         float radius, float collision_range, float x_min, float x_max, float y_min, float y_max)
...
//     // Construct ROS subscribers and publishers
//     _velocity_sub = nh.subscribe("turtle"+std::to_string(_id)+"/sim/cmd_vel", queue_size, &SimTurtle::velocity_callback, this);
//     _pose_pub = nh.advertise<geometry_msgs::Pose2D>("turtle"+std::to_string(_id)+"/sim/pose", queue_size);
//     _sensors_pub = nh.advertise<io_turtle_msgs::DistanceSensor>("turtle"+std::to_string(_id)+"/sim/sensors", queue_size);

//     _teleport_service = nh.advertiseService("turtle"+std::to_string(_id)+"/teleport_turtle", &SimTurtle::teleport_turtle, this);
//     ROS_INFO("'Teleport turtle' service started.");

// }

// void SimTurtle::update_pose(float time_step) {
//     SimTurtleBase::update_pose(time_step);
//     _pose_pub.publish(_turtle._pose);
// }

// void SimTurtle::update_sensor(std::unordered_map<int, SimTurtle>& turtles) {
//     SimTurtleBase::update_sensor<SimTurtle>(turtles);
//     _sensors_pub.publish(_turtle._sensors);
// }


// bool SimTurtle::teleport_turtle(TeleportTurtle::Request& req, TeleportTurtle::Response& response) {
//     response.data = set_pose(req.x, req.y, req.theta);
//     if(response.data){
//         ROS_INFO("Teleported Turtle to (%f, %f, %f)", req.x, req.y, req.theta);
//     }
    
//     return response.data;
// }

// void SimTurtle::velocity_callback(const geometry_msgs::Twist& vel) {
...
// }  // namespace turtlesim Initialize parameters
//         priv_nh.param("queue_size", queue_size, 100);
